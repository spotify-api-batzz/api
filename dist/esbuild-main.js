(() => {
  ...
  var schema = import_joi.default.object({
    limit: import_joi.default.number(),
    offset: import_joi.default.number(),
    joins: import_joi.default.array().items(import_joi.default.string().valid(...Object.keys(models)))
  });
  Object.keys(models).forEach((key) => {
    app.get(`/${key}`, async (req, res) => {
      const joins = req.query.joins ? (0, import_ramda.unnest)(req.query.joins.split(",").map((join) => join.split("."))) : [];
      console.log(joins);
      const {error, warning} = schema.validate(__spreadProps(__spreadValues({}, (0, import_ramda.omit)(["joins"], req.query)), {
        joins
      }));
      if (error) {
        res.send(error);
      ...
      console.log(parseIncludes(req.query.joins));
      let items = await models[key].findAll(__spreadValues(__spreadValues({}, (0, import_ramda.pick)(["limit", "offset"], req.query)), parseIncludes(req.query.joins)));
      res.send(items);
    });
  });
  app.listen(3e3);
